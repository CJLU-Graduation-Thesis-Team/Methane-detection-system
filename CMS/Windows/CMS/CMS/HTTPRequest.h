...
public:
	...
	bool CHTTPRequest::GetUrlData(HTTP_METHOD enumMetMod, std::wstring wstrUrlObject , std::vector<std::string>& vecRetData);  //返回客户端请求是附带的数据  (HTTP头中只有ANSI字符,所以返回string向量集).

	bool CHTTPRequest::PostUrlData(std::wstring wstrUrlObject, std::vector<std::string>& vecRetData);  //返回客户端请求是附带的数据  POST
