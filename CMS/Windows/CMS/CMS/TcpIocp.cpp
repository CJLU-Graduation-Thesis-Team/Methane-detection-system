...
{    
	...
	LOGGER_CINFO(theLogger, _T("设备连接工作者线程启动，ID: %d..\r\n"), nThreadNo );
...
{    
	...
	if (NO_ERROR != nResult)
	{
		LOGGER_CINFO(theLogger, _T("初始化WinSock 2.2失败！.\r\n"));
		return false; 
	}
...
bool CIOCPModel::Start()
{
	...
	{
		LOGGER_CINFO(theLogger, _T("初始化IOCP失败！..\r\n"));
		return false;
	}
	else
	{
		LOGGER_CINFO(theLogger, _T("初始化Tcp IOCP完毕！..\r\n"));
	}

	// 初始化Socket
	if( false==_InitializeListenSocket() )
	{
		LOGGER_CINFO(theLogger, _T("Listen Socket初始化失败！..\r\n"));
		this->_DeInitialize();
		return false;
	}
	else
	{
		LOGGER_CINFO(theLogger, _T("Listen Socket初始化完毕！..\r\n"));
	}
	LOGGER_CINFO(theLogger, _T("设备连接系统准备就绪，等候设备连接......\r\n"));
...
void CIOCPModel::Stop()
{
	if( m_pListenContext!=NULL && m_pListenContext->m_Socket!=INVALID_SOCKET )
	{
		...
		LOGGER_CINFO(theLogger, _T("设备端口停止监听.\r\n"));
...
bool CIOCPModel::_InitializeIOCP()
{
	...
	{
		LOGGER_CINFO(theLogger, _T("建立完成端口失败！错误代码: %d.\r\n") , WSAGetLastError() );
		return false;
	}
	...
	LOGGER_CINFO(theLogger, _T("建立DevTcp Iocp  _WorkerThread %d 个 .\r\n"), m_nThreads );
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	{
		LOGGER_CINFO(theLogger, _T("初始化Socket失败，错误代码: %d .\r\n"), WSAGetLastError());
		return false;
	}
	else
	{
	...
	{  
		LOGGER_CINFO(theLogger, _T("绑定 Listen Socket至完成端口失败！错误代码: %d .\r\n"), WSAGetLastError());
	...
	{
		LOGGER_CINFO(theLogger, _T("bind()函数执行错误.\r\n"));
		return false;
	}
	else
	{
	...
	{
		LOGGER_CINFO(theLogger, _T("Listen()函数执行出现错误.\r\n"));
		return false;
	}
	else
	{
	...
	if(SOCKET_ERROR == WSAIoctl(
		...
		NULL))  
	{  
		LOGGER_CINFO(theLogger, _T("WSAIoctl 未能获取GuidGetAcceptExSockAddrs函数指针。错误代码: %d .\r\n") , WSAGetLastError());
		this->_DeInitialize();
		return false; 
	}  
	...
	LOGGER_CINFO(theLogger, _T("投递 %d 个AcceptEx请求完毕.\r\n"), MAX_POST_ACCEPT);
...
void CIOCPModel::_DeInitialize()
{
	...
	LOGGER_CINFO(theLogger, _T("设备连接工作者线程释放资源完毕..\r\n"));
...
bool CIOCPModel::_PostAccept( PER_IO_CONTEXT* pAcceptIoContext )
{
	...
	{  
		...
		LOGGER_CINFO(theLogger, _T("创建用于Accept的Socket失败！错误代码: %d..\r\n"), WSAGetLastError());
	...
	{  
		...
		{  
			...
			LOGGER_CINFO(theLogger, _T("投递 AcceptEx 请求失败，错误代码: %d..\r\n"), WSAGetLastError());
...
bool CIOCPModel::_PostRecv( PER_IO_CONTEXT* pIoContext )
{
	...
	{
		LOGGER_CINFO(theLogger, _T("投递第一个WSARecv失败！.\r\n"));
...
bool CIOCPModel::_AssociateWithIOCP( PER_SOCKET_CONTEXT *pContext )
{
	...
	if (NULL == hTemp)
	{
		LOGGER_CINFO(theLogger, _T("设备连接线程执行CreateIoCompletionPort()出现错误.错误代码：%d..\r\n"), GetLastError() );
...
bool CIOCPModel::HandleError( PER_SOCKET_CONTEXT *pContext,const DWORD& dwErr )
{
	...
	{  	
		...
		{
			LOGGER_CINFO(theLogger, _T("设备连接线程 检测到甲烷检测设备异常退出.\r\n"));
			this->_RemoveContext( pContext );
			return true;
		}
		else
		{
			LOGGER_CINFO(theLogger, _T("设备连接线程 网络操作超时！重试中....\r\n"));
			return true;
		}
	}  
	...
	{
		LOGGER_CINFO(theLogger, _T("设备连接线程 完成端口操作出现错误，线程退出。错误代码：%d ..\r\n"), dwErr);
