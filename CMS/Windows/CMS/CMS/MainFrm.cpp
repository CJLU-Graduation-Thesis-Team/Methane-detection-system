...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	_tcscpy(m_IconData.szTip, _T("甲烷检测云平台 - by Guangle's C++ Studio"));
	...
	{
		LOGGER_CINFO(theLogger, _T("甲烷检测云平台 启动中...\r\n"));
		OnStart();
	}
	else
	{
		LOGGER_CINFO(theLogger, _T("甲烷检测云平台 准备就绪.\r\n"));
	}
...
void CMainFrame::OnStart()
{
	...
	{
		LOGGER_CINFO(theLogger, _T("加载Winsock 2.2失败，服务器端无法运行！..\r\n"));
	}
	if (true == m_Tcp.Start())
	{
		LOGGER_CINFO(theLogger, _T("Tcp Iocp 启动成功..\r\n"));
	}
	else
	{
		LOGGER_CINFO(theLogger, _T("Tcp Iocp 启动失败..\r\n"));
	}
